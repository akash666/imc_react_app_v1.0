...
class App extends Component {
  ...
  CreateSlots = debounce(
    (rows,columns,target) => {
      ...
      try {
        ...
        tbl.setAttribute('id', '_tableSlots');
  ...
  SlotClearing = (currentSlotID) => {
    ...
    try {
      ...
      var child = document.getElementById(currentSlotID +"_divEventPlayer");
      child.parentNode.removeChild(child);
      child = document.getElementById(currentSlotID +"_acktags");
      child.parentNode.removeChild(child);
    ...
    try 
    {
      ...
      for (let i = 0; i < 2; i++) {
        ...
        for (let j = 0; j < 2; j++) {
          ...
          var btn = this.CreateButton('TerTags', id, "Suspicious Activity"); // <EventPlayer QueueEvent={QueueEvent} />
          render(btn, cell);
          row.appendChild(cell);
        }
        tblBody.appendChild(row);
  ...
  PlaceEvent(CameraEvent){
    ...
    try {
      ...
      //setTimeout(() => {
        ...
        console.log("available slot: ", element);
  ...
  componentDidMount() 
  {
    var target = document.getElementById("_DivContainer");
    var Rows = 2;
    var Columns = 2;
  ...
  render(){
    return (
      <div className="App">
        ...
        <button onClick={() => this.onButtonAckMessageClicked(4)}>Ack Message</button>
        <div >
          <div className='DivSlots' id='_DivContainer'></div>
        </div>
      </div>
    );
  }
